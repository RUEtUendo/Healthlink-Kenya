        ...
        <div class="sw-card">
          ...
          <div style="position:relative;height:130px;margin-bottom:8px;">
            <svg width="100%" height="130" viewBox="0 0 300 130">
              ...
              <text x="10" y="130" fill="rgba(255,255,255,0.3)" font-size="9">0km</text>
          ...
          <table class="triage-table" id="db-table">
            <thead>
              <tr>
                ...
                <th>Age</th>
                <th>Gender</th>
                <th onclick="sortDB('subcounty')" style="cursor:pointer;">Sub-county ↕</th>
                <th>Condition</th>
                <th onclick="sortDB('risk')" style="cursor:pointer;">Risk ↕</th>
                <th>Distance (km)</th>
                <th>Insurance</th>
                <th>Last Visit</th>
                <th>Assigned SW</th>
...
async function loadHubStats() {
  ...
  if (s) {
    ...
    document.getElementById('hub-active-routes').textContent = s.active_routes || "0";
    document.getElementById('hub-avg-retention').textContent = (s.avg_retention_score || 0) + "%";
  }
}
 
async function loadPatientsFromAPI() {
  try {
    const res = await fetch('http://127.0.0.1:8000/patients/?page_size=100');
...
async function loadPatients() {
  if(window.PRELOADED_PATIENTS && window.PRELOADED_PATIENTS.length > 0) {
    allPatients = window.PRELOADED_PATIENTS;
    dbFiltered = [...allPatients];
    initDB();
    return;
  }
...
allPatients.length;
 ...
 document.getElementById('db-nhif-stat').textContent = nhif;
...
function sortDB(key) {
 ...
 dbFiltered.sort((a,b) => {
   ...
   return va < vb ? -dbSortDir : va > vb ? dbSortDir : 0;
  });
 renderDBTable();
}
 
function dbChangePage(dir) {
 const total = Math.ceil(dbFiltered.length / dbPageSize);
 dbCurrentPage = Math.max(1, Math.min(total, dbCurrentPage + dir));
 renderDBTable();
}
 
function renderDBTable() {
 const start = (dbCurrentPage - 1) * dbPageSize;
 const rows = dbFiltered.slice(start, start + dbPageSize);
 const total = Math.ceil(dbFiltered.length / dbPageSize);
 document.getElementById('db-page-info').textContent = `Showing ${start+1}–${Math.min(start+dbPageSize, dbFiltered.length)} of ${dbFiltered.length} records · Page ${dbCurrentPage} of ${total}`;
 document.getElementById('db-prev').disabled = dbCurrentPage === 1;
 document.getElementById('db-next').disabled = dbCurrentPage === total || total === 0;
 document.getElementById('db-tbody').innerHTML = rows.length ? rows.map(p => {
   const rc = p.risk === 'High' ? 'risk-h' : p.risk === 'Medium' ? 'risk-m' : 'risk-l';
   const insc = p.ins === 'NHIF' ? 'color:#6EE7B7;' : p.ins === 'Partial' ? 'color:#FCD34D;' : 'color:#F87171;';
   return `<tr>
     <td style="font-family:monospace;color:var(--amber);font-size:12px;">${p.id}</td>
     <td style="font-weight:600;color:white;">${p.name}</td>
     <td>${p.age}</td>
     <td>${p.gender === 'F' ? '♀' : '♂'} ${p.gender}</td>
     <td><span style="background:rgba(29,158,117,0.15);color:var(--teal-mid);padding:2px 8px;border-radius:4px;font-size:11px;">${p.sub}</span></td>
     <td style="color:rgba(255,255,255,0.7);">${p.cond}</td>
     <td><span class="risk-pill ${rc}">${p.risk}</span></td>
     <td style="color:${p.dist > 35 ? '#F87171' : 'rgba(255,255,255,0.7)'};">${p.dist}</td>
     <td style="${insc}font-weight:600;">${p.ins}</td>
     <td style="color:rgba(255,255,255,0.55);font-size:12px;">${p.lastVisit}</td>
     <td style="color:rgba(255,255,255,0.45);font-size:12px;">${p.sw}</td>
     <td><button class="triage-action-btn" onclick="openTriageModal()">View</button></td>
   </tr>`;
 }).join('') : `<tr><td colspan="12" style="text-align:center;color:rgba(255,255,255,0.35);padding:32px;">No patients match the current filters</td></tr>`;
}
...
async function apiGet(endpoint) {
    if (endpoint === '/analytics/dashboard/stats') {
        return new Promise(resolve => setTimeout(() => resolve({
            high_risk_count: 1284,
            evasive_profiles: 347,
            active_routes: 23,
            avg_retention_score: 68
        }), 500));
    }
    return null;
}
